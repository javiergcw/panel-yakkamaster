...
const getJobByIdUseCase = new GetJobByIdUseCase(jobsService);
const updateJobUseCase = new UpdateJobUseCase(jobsService);
...
export default function EditJobPage() {
  ...
  useEffect(() => {
    if (activeStep === 0) loadJobsites();
  }, [activeStep, loadJobsites]);

  useEffect(() => {
    if (activeStep === 1) loadSkills();
  ...
  // Load job by id and optionally skills/jobsites to populate form
  useEffect(() => {
    if (!id) {
      setJobLoading(false);
    ...
    setJobLoading(true);
    setJobError(null);
    Promise.all([getJobByIdUseCase.execute(id), getSkillsUseCase.execute(), getJobsitesUseCase.execute()])
      .then(([jobData, skillsResp, jobsitesResp]) => {
        setJob(jobData);
        setJobsitesData(jobsitesResp);
        setSkillsData(skillsResp);
        setJobsitesLoading(false);

        // Detail API may return jobsite/job_type as nested objects
        const jobsiteId = jobData.jobsite_id ?? (jobData as { jobsite?: { id?: string } }).jobsite?.id;
        const jobTypeId = jobData.job_type_id ?? (jobData as { job_type?: { id?: string } }).job_type?.id;

        setSelectedJobsite(jobsiteId ?? null);
        setJobType(jobTypeId ?? null);
        setWorkersAmount(String(jobData.many_labours ?? 1));
        setWage(Number(jobData.wage_hourly_rate).toFixed(2));
        setSiteAllowance(Number(jobData.wage_site_allowance).toFixed(2));
        setLeadingHandAllowance(Number(jobData.wage_leading_hand_allowance).toFixed(2));
        setProductivityAllowance(Number(jobData.wage_productivity_allowance).toFixed(2));
        setOvertimeRate(String(jobData.extras_overtime_rate ?? 0));
        setTravelAllowance(Number(jobData.travel_allowance).toFixed(2));

        setStartDate(jobData.start_date_work ? new Date(jobData.start_date_work) : null);
        setEndDate(jobData.end_date_work ? new Date(jobData.end_date_work) : null);
        setIsOngoing(Boolean(jobData.ongoing_work));
        setWorkSaturdays(Boolean(jobData.work_saturday));
        setWorkSundays(Boolean(jobData.work_sunday));

        setStartTime(jobData.start_time ? parseTimeToDate(jobData.start_time) : null);
        setEndTime(jobData.end_time ? parseTimeToDate(jobData.end_time) : null);
        setDescription(jobData.description || "");
        setLicenses((jobData.job_licenses ?? []).map((l) => l.license_id).filter(Boolean));
        setShowJobPublic(jobData.visibility === "PUBLIC");
        setPayDay(jobData.payment_day ? new Date(jobData.payment_day) : null);
        setPaymentSchedule("choose_pay_day");

        // job_requirements from detail API: array of { id, name, ... }
        const reqIds = (jobData as { job_requirements?: { id: string }[] }).job_requirements?.map((r) => r.id) ?? [];
        setJobRequirements(reqIds);

        const firstSkill = jobData.job_skills?.[0] as
          | { skill_category_id?: string; skill_subcategory_id?: string; skill_category?: { id: string; name: string }; skill_subcategory?: { id: string; name: string } }
          | undefined;
        if (firstSkill && skillsResp?.data) {
          const catId = firstSkill.skill_category_id ?? firstSkill.skill_category?.id;
          const subId = firstSkill.skill_subcategory_id ?? firstSkill.skill_subcategory?.id;
          const cat = skillsResp.data.find((c) => c.id === catId);
          if (cat) {
            const sub = cat.subcategories.find((s) => s.id === subId);
            if (sub) setSelectedSkill(skillDisplayLabel(cat.name, sub.name));
          } else if (firstSkill.skill_category && firstSkill.skill_subcategory) {
            setSelectedSkill(skillDisplayLabel(firstSkill.skill_category.name, firstSkill.skill_subcategory.name));
          }
        }
      })
      .catch((err) =>
  ...
  function buildPayload(): CreateJobRequest | null {
    if (!id || !selectedJobsite || !jobType || !workersAmount || !startDate || !endDate || !paymentSchedule)
      return null;
    ...
    const gst = wageHourlyRate * 0.10 * 0.10;
  ...
  const handleClose = () => router.push("/dashboard/jobs");

  const handleNext = () => {
    if (activeStep < 8) setActiveStep((s) => s + 1);
    else if (activeStep === 8 && id) {
  ...
  if (jobLoading) {
    return (
      <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: 400 }}>
        <Typography sx={{ color: "#86868b" }}>Loading job...</Typography>
      </Box>
    );
  }
  ...
  if (jobError || !job) {
    return (
      <Box sx={{ p: 4, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
        <Typography sx={{ color: "error.main" }}>{jobError ?? "Job not found"}</Typography>
        <Button onClick={() => router.push("/dashboard/jobs")} variant="outlined">
          Back to Jobs
        </Button>
      </Box>
    );
    ...
    >
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography sx={{ fontWeight: 900, letterSpacing: "1px", color: "#1d1d1f", fontSize: { xs: "1rem", sm: "1.25rem" } }}>
            YAKKA SPORTY
          </Typography>
        </Box>
        <IconButton onClick={handleClose} sx={{ bgcolor: "#f5f5f7", "&:hover": { bgcolor: "#e8e8ed" }, width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 } }}>
          <CloseIcon sx={{ color: "#1d1d1f", fontSize: 20 }} />
        </IconButton>
      </Box>

      <Box sx={{ flexGrow: 1, display: "flex", flexDirection: "column" }}>{renderStepContent()}</Box>

      <CreateJobsiteModal open={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} onSuccess={loadJobsites} />
      ...
      >
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1 }}>
          <Typography variant="h6" sx={{ fontWeight: 800, color: "#1d1d1f" }}>Custom amount</Typography>
          <IconButton onClick={() => setIsCustomAmountModalOpen(false)} size="small">
        ...
        <DialogActions sx={{ p: 3, pt: 0 }}>
          <Button
            ...
            sx={{
              ...
              textTransform: "none",
              boxShadow: "none",
              "&:hover": { bgcolor: "#5cb85c", boxShadow: "none" },
              "&.Mui-disabled": { bgcolor: "#f5f5f7", color: "#bdbdbd" },
            }}
          >
            Confirm
          </Button>
